...
class Ensemble(nn.Module):
    def __init__(self, opts):
        ...
        self.stargan = StarG(conv_dim=self.conv_dim, c_dim=self.c_dim, repeat_num=self.repeat_num).to(config.device)
        self.load_weights()
        # load attgan
    ...
    def compute_ensemble_out_loss(self, x_real, c_trg_list, b_trg_list):
        outs_fgan, outs_attgan, outs_hisd = self.ensemble_models_out(x_real, c_trg_list, b_trg_list)
    ...
    def compute_stargan_loss(self, x_adv, x_real, c_trg_list):
        criterion = torch.nn.MSELoss()
        ...
        for i, c_trg in enumerate(c_trg_list):
            ori_out, ori_mids = self.stargan(x_real, c_trg)
            adv_out, adv_mids = self.stargan(x_adv, c_trg)
        ...
        mse = criterion(ori_out, adv_out)

        cosine_sim = cosine_loss(ori_out.flatten(1), adv_out.flatten(1)).mean()
        cosine_diff = 1 - cosine_sim

        gdl_feature = self.sobel_gradient_difference_loss(feature_ori, feature_adv, alpha=0.1)

        total_loss = -(
                mse * 1e4 +   # 主要通过MSE计算视觉质量损失
                cosine_diff * 1e2 +
                gdl_feature * 1e2
        )
    ...
    def compute_fan_loss(self, x_adv, x_real, c_trg_list):
        criterion = torch.nn.MSELoss()
        cosine_loss = torch.nn.CosineSimilarity(dim=1)
        ...
        mse = criterion(ori_mid, adv_mid)

        cosine_sim = cosine_loss(ori_mid.flatten(1), adv_mid.flatten(1)).mean()
        cosine_diff = 1 - cosine_sim

        gdl_feature = self.sobel_gradient_difference_loss(ori_mid, adv_mid, alpha=0.1)
    ...
    def sobel_gradient_difference_loss(self, ori_img, adv_img, alpha=1.0):
        sobel_x = torch.tensor([[[[1, 0, -1], [2, 0, -2], [1, 0, -1]]]],
                               dtype=torch.float32).to(ori_img.device)
        sobel_y = torch.tensor([[[[1, 2, 1], [0, 0, 0], [-1, -2, -1]]]],
                               dtype=torch.float32).to(ori_img.device)

        def compute_gradient(img, kernel):
            gradients = []
            for c in range(img.shape[1]):
                channel_grad = F.conv2d(img[:, c:c + 1], kernel, padding=1)
                gradients.append(channel_grad)
            return torch.cat(gradients, dim=1)

        ori_grad_x = compute_gradient(ori_img, sobel_x)
        ori_grad_y = compute_gradient(ori_img, sobel_y)
        adv_grad_x = compute_gradient(adv_img, sobel_x)
        adv_grad_y = compute_gradient(adv_img, sobel_y)

        grad_diff_x = torch.abs(torch.abs(ori_grad_x) - torch.abs(adv_grad_x))
        grad_diff_y = torch.abs(torch.abs(ori_grad_y) - torch.abs(adv_grad_y))

        gdl_loss = torch.mean(grad_diff_x) + torch.mean(grad_diff_y)

        return gdl_loss * alpha

    def compute_attgan_loss(self, x_adv, x_real):
        mse_loss = torch.nn.MSELoss()
        cosine_loss = torch.nn.CosineSimilarity(dim=1)

        feature_ori = self.attgan.G(x_real, mode='enc')[-1]
        feature_adv = self.attgan.G(x_adv, mode='enc')[-1]

        ori_mid = F.interpolate(self.conv2(feature_ori), size=(64, 64),
                                mode='bilinear', align_corners=False)
        adv_mid = F.interpolate(self.conv2(feature_adv), size=(64, 64),
                                mode='bilinear', align_corners=False)

        mse = mse_loss(ori_mid, adv_mid)
        cosine_sim = cosine_loss(ori_mid.flatten(1), adv_mid.flatten(1)).mean()
        cosine_diff = 1 - cosine_sim
        gdl_feature = self.sobel_gradient_difference_loss(ori_mid, adv_mid, alpha=0.1)
    ...
    def compute_hisd_loss(self, x_adv, x_real):
        criterion = torch.nn.MSELoss()
        mse_loss = torch.nn.MSELoss()
        cosine_loss = torch.nn.CosineSimilarity(dim=1)
        ...
        s_trg_black = self.F_(self.reference_black, 2)
        ori_mids_2 = self.T(c_ori, s_trg_black, 2)

        s_trg_blond = self.F_(self.reference_blond, 2)
        ori_mids_3 = self.T(c_ori, s_trg_blond, 2)

        s_trg_brown = self.F_(self.reference_brown, 2)
        ori_mids_4 = self.T(c_ori, s_trg_brown, 2)

        s_trg_bands = self.M(self.w, 0, 0)
        ori_mids_5 = self.T(c_ori, s_trg_bands, 0)

        ori_mids = (ori_mids_1 + ori_mids_2 + ori_mids_3 + ori_mids_4 + ori_mids_5) / 5.0

        c_adv = self.E(x_adv)
        adv_mids_1 = self.T(c_adv, s_trg_glass, 1)
        adv_mids_2 = self.T(c_adv, s_trg_black, 2)
        adv_mids_3 = self.T(c_adv, s_trg_blond, 2)
        adv_mids_4 = self.T(c_adv, s_trg_brown, 2)
        adv_mids_5 = self.T(c_adv, s_trg_bands, 0)

        adv_mids = (adv_mids_1 + adv_mids_2 + adv_mids_3 + adv_mids_4 + adv_mids_5) / 5.0
        mse = mse_loss(ori_mids, adv_mids)
        cosine_sim = cosine_loss(ori_mids.flatten(1), adv_mids.flatten(1)).mean()
        cosine_diff = 1 - cosine_sim
        gdl_feature = self.sobel_gradient_difference_loss(ori_mids, adv_mids, alpha=0.1)

        total_loss = -(
                mse * 1e4 +  # 放大MSE损失
                cosine_diff * 1e2 +  # 余弦差异损失
                gdl_feature * 1e2  # 特征空间梯度差异
        )

        # return -criterion(ori_mids, adv_mids)
        return total_loss

    def ensemble_compute_loss(self, x_adv, x_real, c_trg_list, b_trg_list, weights_softmax):
        criterion = torch.nn.MSELoss()
        mse_loss = torch.nn.MSELoss()
        cosine_loss = torch.nn.CosineSimilarity(dim=1)

        if not hasattr(self, 'lpips_loss'):
            self.lpips_loss = lpips.LPIPS(net='alex').to(x_real.device)
            # 冻结LPIPS参数
        ...
        outs_fgan, outs_attgan, outs_hisd = self.ensemble_models_out(x_real, c_trg_list, b_trg_list)
        outs_fgan_adv, outs_attgan_adv, outs_hisd_adv = self.ensemble_models_out(x_adv, c_trg_list, b_trg_list)
        avg_fgan = torch.stack(outs_fgan).mean(dim=0)
        avg_attgan = torch.stack(outs_attgan).mean(dim=0)
        avg_hisd = torch.stack(outs_hisd).mean(dim=0)
        out_avg = (avg_fgan + avg_attgan + avg_hisd) / 3.0

        avg_fgan_adv = torch.stack(outs_fgan_adv).mean(dim=0)
        avg_attgan_adv = torch.stack(outs_attgan_adv).mean(dim=0)
        avg_hisd_adv = torch.stack(outs_hisd_adv).mean(dim=0)

        out_avg_adv = (avg_fgan_adv + avg_attgan_adv + avg_hisd_adv) / 3.0

        # loss = -criterion(feature_ori, feature_adv) * 1e7

        #  视觉质量损失
        # mse = mse_loss(out_avg, out_avg_adv)
        mse = mse_loss(feature_ori, feature_adv)

        # lpips_val = self.lpips_loss(feature_ori, feature_adv)
        cosine_sim = cosine_loss(feature_ori.flatten(1), feature_adv.flatten(1)).mean()
        cosine_diff = 1 - cosine_sim
    ...
    def ensemble_compute_loss_nomerge(self, x_adv, x_real, c_trg_list, weights_softmax):
        ...
        # ori_mid1 = vit_tools.nn.functional.interpolate(self.conv2(feature_ori1), size=(64, 64), mode='bilinear',
        #                                            align_corners=False)
        # adv_mid1 = vit_tools.nn.functional.interpolate(self.conv2(feature_adv1), size=(64, 64), mode='bilinear',
        #                                            align_corners=False)
        ...
        s_trg_bands = self.M(self.w, 0, 0)  # hair
        ori_mids_5 = self.T(c_ori, s_trg_bands, 0)
        ori_mids = (ori_mids_1 + ori_mids_2 + ori_mids_3 + ori_mids_4 + ori_mids_5) / 5.
        # hisd adv1
        c_adv1 = self.E(x_adv)
